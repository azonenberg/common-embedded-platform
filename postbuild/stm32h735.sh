...
ITCMSTART=$(arm-none-eabi-objdump -t $1  | grep __itcm_start | cut -d ' ' -f 1);
ITCMEND=$(arm-none-eabi-objdump -t $1  | grep __itcm_end | cut -d ' ' -f 1);
TCMSTART=$(arm-none-eabi-objdump -t $1  | grep __tcmstart | cut -d ' ' -f 1);
